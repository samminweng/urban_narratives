...
function HighlightTextView(_text, _words) {
    const text = _text;
    const words = _words;
    // The container
    ...
    function _createUI() {
        ...
        let subjectPhrase = text['subject_phrase'];
        ...
        let objectPhrase = text['object_phrase'];
        // console.log(words);
        // Get the selected words from
        let objectIds = [];
        let objectPhraseClouds = text["object_phrase_cloud"].filter(p => p['lemmaWords'] === words);
        if (objectPhraseClouds.length > 0) {
            let objectTerms = objectPhraseClouds[0]['phraseWords'];
            for (let objectTerm of objectTerms) {
                highlightWord(objectTerm, "object-Term", textContainer);
                objectIds.push(objectTerm['id']);
            }
        }

        // Highlight the object terms if needed
        // let objectCategory = text['objectCategory'];
        let objectTerm = text['object_category']['term'];
        ...
        // Filter out object
        objectPhrase = objectPhrase.filter(word => !objectIds.includes(word['id']));
